...
class Hasil extends Koneksi
{
    ...
    {
        $sql = "SELECT COUNT(id) AS count FROM alternatif";
    ...
    {
        $sql = "SELECT MAX(id) AS count FROM alternatif";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
        $data = [];
        while ($rows = $stmt->fetch()) {
    ...
    public function spk($b)
    {
        //Initilize

        $sql = "SELECT * FROM alternatif INNER JOIN kategory ON alternatif.id_kategori = kategory.id_kategori WHERE id = :id";
        $stmt = $this->db->prepare($sql);
        $stmt->bindparam(":id", $b);
        $stmt->execute();
        //Fetch
    ...
    public function MMULT($array1 = array(), $val = 0)
    {
        $hasil = new Hasil();
    ...
    public function hitung($val = 0)
    {
        ...
        do {
            ...
            foreach ($roww as $rows) {
                ...
                $alternatif = $rows['id'];
                $altname = $rows['ahass'];
                $target3 = round((($sales - $rows['t3']) / $rows['t3']) * 100, 0);
                $target2 = $mekanik - $rows['t2'];

                if (round(($unit / $rows['t1']) * 100, 0) > 105) {
                    $nilai1 = 8;
                    $fault1 = 40;
                } elseif (round(($unit / $rows['t1']) * 100, 0) > 100) {
                    $nilai1 = 7;
                    $fault1 = 30;
                } elseif (round(($unit / $rows['t1']) * 100, 0) > 95) {
                    $nilai1 = 6;
                    $fault1 = 20;
                } elseif (round(($unit / $rows['t1']) * 100, 0) > 90) {
                    $nilai1 = 5;
                    $fault1 = 15;
                } elseif (round(($unit / $rows['t1']) * 100, 0) > 85) {
                    $nilai1 = 4;
                    $fault1 = 10;
                } elseif (round(($unit / $rows['t1']) * 100, 0) > 80) {
                    $nilai1 = 3;
                    $fault1 = 5;
                } elseif (round(($unit / $rows['t1']) * 100, 0) > 70) {
                    $nilai1 = 2;
                    $fault1 = 0;
                } else {
                    $nilai1 = 1;
                ...
                if ($buka == 4 && $buka_ue >= $rows['t5']) {
                    $nilai5 = 5;
                    $fault5 = 15;
                } elseif ($buka == 4 && $buka_ue < $rows['t5']) {
                    $nilai5 = 4;
                    $fault5 = 10;
                } elseif ($buka >= 2  && $buka_ue >= $rows['t5']) {
                    $nilai5 = 4;
                    $fault5 = 10;
                } elseif ($buka >= 2 && $buka_ue < $rows['t5']) {
                    $nilai5 = 3;
                    $fault5 = 5;
                } elseif ($buka == 1 && $buka_ue >= $rows['t5']) {
                    $nilai5 = 3;
                    $fault5 = 5;
                } elseif ($buka == 1 && $buka_ue < $rows['t5']) {
                    $nilai5 = 2;
